...
namespace Unity.DemoTeam.DigitalHuman
{
    ...
    public class LegacySkinAttachment : MeshInstanceBehaviour
    {
        ...
        [EditableIf("attachmentType", AttachmentType.MeshRoots)]
        public bool allowOnlyOneRoot = false;

        [HideInInspector] public ulong checksum0 = 0;
        [HideInInspector] public ulong checksum1 = 0;
        ...
        public void NotifyOfMeshModified(CommandBuffer cmd)
        {
                ...
                if (showRootLines)
                {
                    ...
                    Matrix4x4 targetToWorld;
                    {
                        ...
                        if (target.meshBakedSmr != null)
                            targetToWorld = Matrix4x4.TRS(target.transform.position, target.transform.rotation,
                                Vector3.one);
                        else
                    ...
                    {
                        if (skinningBone != null)
                            targetToSubject = (skinningBoneBindPoseInverse * skinningBone.worldToLocalMatrix) *
                                              targetToWorld;
                        else
